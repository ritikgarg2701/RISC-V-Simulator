...
def printAtSimulationEnd():
    ...
    print("Total number of instructions executed = ", InstructionsExecuted - 1)
...
def staller():
    global numberOfStalls, dataHazzardStalls, dataHazzards
...
def hazardDetectionUnitPrevPrev():
    global rs1, rs2, prevPrevIns, MuxASelect, MuxRMSelect, MuxBSelect, dataHazzards, dataHazzardStalls, prevIns, prevPrevIns, prevPrevPrevIns, curIns
    if (rs1 != ""):
...
def hazardDetectionUnitPrev():
    global prevIns, prevPrevIns, prevPrevPrevIns, curIns, dataHazzardStalls, dataHazzards, numberOfStalls, MuxASelect, MuxBSelect, isStall, MuxMSelect, dataStallCount, MuxRMSelect, isDependentPrev, stallCount
    isDependentPrev = False
    # E to E
    # M to E
    # M to M
...
def hazardDetectionUnitControlPrevPrevPrev():
    global dataHazzards, MuxRASelect, MuxRBSelect, prevIns, prevPrevIns, prevPrevPrevIns, curIns
    if (rs1 != ""):
...
def hazardDetectionUnitControlPrevPrev():
    global prevIns, prevPrevIns, prevPrevPrevIns, curIns, dataHazzardStalls, dataHazzards, controlHazzards, numberOfStalls, MuxRASelect, MuxRBSelect, stallCount, controlEDType, isControlStall, controlHazzardStalls
    if (rs1 != ""):
        if (rs1 == prevPrevIns[1] and rs1 != "00000"):
            if (prevPrevIns[0] == '0'):
                MuxRASelect = 1
...
def hazardDetectionUnitControlPrev():
    ...
    if (rs1 != ""):
        if (rs1 == prevIns[1] and rs1 != "00000"):
            ...
            if (prevIns[0] == '0'):
                # MuxRASelect = 1
                # E to D forwarding with one stall ( Ho gya hue hue )
                prevPrevPrevIns[1] = prevPrevIns[1]
                prevPrevPrevIns[0] = prevPrevIns[0]
                prevPrevIns[1] = prevIns[1]
                prevPrevIns[0] = prevIns[0]
                ...
                # M to D forwarding with two stalls ( Ho gya hue hue :) )
                # MuxRASelect = 2
                prevPrevPrevIns[1] = prevPrevIns[1]
                prevPrevPrevIns[0] = prevPrevIns[0]
                prevPrevIns[1] = prevIns[1]
...
def memoryAccess():
    ...
    if (ppmemRead == 1):
        ...
        displayMemory += "MEMORY : The value 0x" + str(hex(int("".join(temp1), 2)))[2:].zfill(
        ...
        if (pploadType == 0):  # lb
            ...
            displayMemory += "MEMORY : The value 0x" + str(hex(int("".join(temp2), 2)))[2:].zfill(
                2) + " is loaded from address 0x" + str(hex(int("".join(dec2two(two2dec(MAR) + 1)), 2)))[2:].zfill(
                8) + "\n"

        if (pploadType == 0 or pploadType == 1):
            ...
            displayMemory += "MEMORY : The value 0x" + str(hex(int("".join(temp3), 2)))[2:].zfill(
                2) + " is loaded from address 0x" + str(hex(int("".join(dec2two(two2dec(MAR) + 2)), 2)))[2:].zfill(
                8) + "\n"
        ...
        if (pploadType == 0 or pploadType == 1):
            ...
            displayMemory += "MEMORY : The value 0x" + str(hex(int("".join(temp4), 2)))[2:].zfill(
                2) + " is loaded from address 0x" + str(hex(int("".join(dec2two(two2dec(MAR) + 3)), 2)))[2:].zfill(
                8) + "\n"
    ...
    if (len(sys.argv) > 2):
        ...
        try:
            ...
            if (flag == '1'):
                ...
                print("Do you want to print the values in the pipeline registers?\n Enter 1 for YES or any other key for NO : ")
